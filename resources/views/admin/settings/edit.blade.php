        ...
        <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            <form action="{{ route('admin.settings.update', $setting) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    @if($setting->link)
                        <div class="mb-2">
                            <a href="{{ asset('storage/' . $setting->link) }}" target="_blank" class="text-blue-500 underline">File hiện tại</a>
                        </div>
                    @endif
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('link') border-red-500 @enderror"
                        ...
                        name="link"
                        accept="*/*">
                    <p class="text-sm text-gray-500 mt-1">Để trống nếu không muốn thay đổi file</p>
                    @error('link')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                    @enderror
                </div>
